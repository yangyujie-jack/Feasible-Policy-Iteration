...
class CSACAgent(SACAgent):
    def __init__(
        ...
        q1_key, q2_key, g1_key, g2_key, policy_key = jax.random.split(key, 5)
    ...
    def get_feasibility(self, obs: np.ndarray, act: np.ndarray) -> np.ndarray:
        g1 = self.q(self.params.g1, obs, act)
        g2 = self.q(self.params.g2, obs, act)
        g = jnp.maximum(g1, g2)
        return np.asarray(g)
