...
class SafeCVFAlgState(NamedTuple):
...
class SafeCVF(SAC):
    def __init__(
        ...
        *,
        ...
        self.alg_state = SafeCVFAlgState(
        ...
        def stateless_update(
            ...
            params: CSACParams,
            alg_state: SafeCVFAlgState,
            data: Experience
        ) -> Tuple[CSACParams, SafeCVFAlgState, dict]:
            obs, action, next_obs, cost, done = (
                data.obs,
                data.action,
                data.next_obs,
                data.cost,
            ...
            g_backup = cost + (1 - done) * self.gamma * g_target
