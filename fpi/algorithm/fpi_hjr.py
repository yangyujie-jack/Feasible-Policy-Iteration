    ...
    def __init__(
        ...
        *,
        ...
        def stateless_update(
            ...
            obs, action, next_obs, reward, done, constraint = (
                data.obs,
                data.action,
                data.next_obs,
                data.reward,
            ...
            def policy_loss_fn(policy_params: hk.Params):
                ...
                g_feas = jnp.clip(g, self.min_constraint, -EPSILON)
                b_coef = self.t * (-1 / jax.lax.stop_gradient(g_feas) + 1 / self.min_constraint)
                feas_loss = (feas * (-q + b_coef * g_feas) / (1 + b_coef) - e).mean()
